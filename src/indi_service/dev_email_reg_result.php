...
<?php
	...
	//ifid은 1
	if ($operid==1)
		$query="insert into dev_serv values($servid,'$userid', '$deviceid', $dev_serv_num, 2, 0, 0, 0, '2016-11-17 11:00:00', 1,  $operid, 'null')";
	else
	{
		$des_email=$_POST['des_email'];
		
		$query="insert into dev_serv values($servid,'$userid', '$deviceid', $dev_serv_num, 2, 0, 0, 0, '2016-11-17 11:00:00', 1,  $operid, '$des_email')";
	}
	$result=mysqli_query($dbc,$query) or die('ss'.$userid.' '.$dev_serv_num);
	
	echo '<pp><h2>'.$deviceid.'에 이메일 서비스 등록이 완료되었습니다</h2></pp>';
